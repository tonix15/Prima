<?php
$page_name = 'Providers with no Rates';
...
$data_list = $dbhandler->valProvidersNoRates($params);
$errmsg = !empty($data_list) ? '':'<strong>No Entries Found.</strong>';	
?>

<div class="sub-menu-title"><h1>Providers with no Rates Report</h1></div>
	<div class="table-wrapper billing-data-list-container">
		...
		<div class="wrapper-panel">	
			<?php 
				...
				ob_start(); 
			?>
			<table class="billing scrollable planning-table planning-table-striped planning-table-hover" style="width:55%;">
				<thead>
					<tr>
				...
				<tbody>
					...
					?>
							<tr>
								<td class="table-column-text-align-center"><?php echo $report['ProviderNumber']; ?></td>								
								<td><?php echo $report['ProviderName']; ?></td>
								<td class="table-column-text-align-right"><?php echo $report['IsInternalProvider']; ?></td>
							</tr>
					<?php endforeach; 
			...
			<?php
				...
				$title = 'Providers with no Rates Report';
