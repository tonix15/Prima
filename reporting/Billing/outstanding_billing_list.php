<?php
$page_name = 'Outstanding Billing List';
...
$data_list = $dbhandler->repOutstandingBillingList($params);
$errmsg = !empty($data_list) ? '':'<strong>No Entries Found.</strong>';	
?>

<div class="sub-menu-title"><h1>Outstanding Billing List Report</h1></div>
	<div class="table-wrapper billing-data-list-container">
		...
		<div class="wrapper-panel">	
			<?php 
				...
				ob_start(); 
			?>
			<table class="billing scrollable planning-table planning-table-striped planning-table-hover" style="width:54%;">
				<thead>
					<tr>												
						<th>Building Number</th>
						<th>PorMan</th>
						<th>Reading Date</th>
						<th>Number of Entered Reading</th>
					</tr>
				</thead>
				<tbody>
					<?php 
					...
					?>
							<tr>								
								<td><?php echo $report['BuildName']; ?></td>
								<td><?php echo $report['PorMan']; ?></td>
								<td class="table-column-text-align-center"><?php echo $report['ReadingDay']; ?></td>
								<td class="table-column-text-align-right"><?php echo $report['ReadCount']; ?></td>
							</tr>
					<?php endforeach; 
			...
			<?php
				...
				$title = 'Outstanding Billing List Report';
