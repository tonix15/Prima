...
if (isset($_GET['View'])) { 	    
	...
	$data_list = $dbhandler->repReadingTestMeter($params);
...
<div class="sub-menu-title"><h1>Meter Test Required Report</h1></div>
<form method="get">
<div id="rate-selection" class="wrapper-fieldset-forms hover-cursor-pointer">
    <fieldset class="fieldset-forms">
        <legend>Reading Test Meter</legend>
        <ul class="fieldset-forms-li-2-cols">            
            <li>Reading Date:</li>
            <li><input type="date" id="reading-test-meter-selection-date" name="choose_date" class="selection-required-input" value="<?php echo $selectedDate; ?>"/></li>	
        </ul>
		...
		<div class="table-wrapper billing-data-list-container">
			...
			<div class="wrapper-panel">	
					...
					<tbody>
						<?php foreach($data_list as $report):?>
							<tr>
								...
								<?php $reading_anchor = DOMAIN_NAME . '/query_and_reporting/Meter/reading.php?choose_building=' . $report['BuildingFk'] . '&choose_unit=' . $report['UnitPk'] . '&choose_meter=' . $report['MeterPk'] . '&View=View'; ?>
				...
				<?php
					...
					$title = 'Meter Test Required Report as of ' . $selectedDate;
